...
string removePunctuation(const string& word) {
	...
	vector <string> url = { "www.", "http://", "https://", ".com", ".xyz", ".lt", ".gov", ".mil", ".arpa", ".edu", ".int", ".net", ".org" };
		...
		while (getline(inFile, line)) {
			...
			while (iss >> word) {
				...
				for (auto i : url) {
					int st = i.size();
					if (st <= end) {
						//if (word.substr(0, st) == i || word.substr(end - st, end) == i) {
...
void findPer(const string& fileName) {
	ifstream inFile(fileName);
	try {
		if (!inFile.is_open()) {
			throw runtime_error("Error: unable to open file: " + fileName);
